...
object ConsoleProducer {
  ...
  class LineMessageReader extends MessageReader {
    ...
    var parseKey = false
    var nullAlias: Option[String] = None
    var keySeparator = "\t"
    ...
    override def init(inputStream: InputStream, props: Properties) {
      ...
      if (props.containsKey("produce.null.alias"))
        nullAlias = Some(props.getProperty("produce.null.alias"))
      if (props.containsKey("key.separator"))
        keySeparator = props.getProperty("key.separator")
      if (props.containsKey("ignore.error"))
        ignoreError = props.getProperty("ignore.error").trim.equalsIgnoreCase("true")

      nullAlias.foreach(alias =>
        System.out.println(
          "produce.null.alias parameter was set. When read value will be \"" + alias + "\", than given key will have null value produced."
        )
      )
      ...
      (reader.readLine(), parseKey) match {
        ...
        case (line, true) =>
          line.indexOf(keySeparator) match {
            ...
            case n =>
              val value: Option[Array[Byte]] = if (n + keySeparator.size > line.size) {
                Some[Array[Byte]]("".getBytes(StandardCharsets.UTF_8))
              } else {
                // if nullAlias is filled in and it is same as value of line after keySeparator, map to null (or else map to empty string bytes
                val valFromInput = line.substring(n + keySeparator.size)
                nullAlias
                  .filter(aliasForNull => aliasForNull == valFromInput)
                  .map(_aliasForNull => None)
                  .getOrElse(Some[Array[Byte]](valFromInput.getBytes(StandardCharsets.UTF_8)))

              }
              val key = line.substring(0, n)
              println("For key=" + key + ", I will produce value=" + value.getOrElse(null))
              new ProducerRecord(topic, key.getBytes(StandardCharsets.UTF_8), value.getOrElse(null))
          }
        case (line, false) =>
